function SNR_plt = plot_sig_noise_111616(subj,cond,plt_cond,elec_list)
  ...
  switch plt_cond
         ...
         case 'trial'
             figure;
               ...
               %take the avg data over all epochs and all channels in each
               %trial
               avg_all_ep = mean(tmp_ep,2);
               for i =1:tot_trl
                   %mean_tmp =  mean(reshape(mean_trl(:,1,i),size(mean_trl(:,1,i),1)/3,3),2);
                    p_spc = power_spect(avg_all_ep(:,1,i),subj.avg_data(cond).sampling_rate);
                    fr = p_spc(:,1);
                    amp_c = p_spc(:,2);
            ...
            text(.1,.45,'Signal (Micro Volt)',...
            'VerticalAlignment','bottom',...
            'HorizontalAlignment','left', 'Rotation', 90, 'FontSize',18);

            text(.45,.05,'Frequency (HZ)',...
            'VerticalAlignment','bottom',...
            'HorizontalAlignment','left', 'Rotation', 0, 'FontSize',18);
      %  SNR per each trial per each epoch
              figure;
              plt_titl = sprintf('%s, %s, %s\n, avg over all channels per trial, SNR, adjacent noise \n ', subj.name,subj.stim_params.cond{cond},subj.date);
              suptitle(plt_titl);  
              num_epochs = size(tmp_ep,2);
              my_ylim1 = [];
              my_ylim2 = [];
              for i =1:tot_trl
                  sig_ep = zeros(2,num_epochs);
                  noise_ep = zeros(2,num_epochs);
                for j =1:num_epochs
                    p_spc = power_spect(tmp_ep(:,j,i),subj.avg_data(cond).sampling_rate);
                    fr = p_spc(:,1);
                    amp_c = p_spc(:,2);
                    sig_ep(1,j) = amp_c(fr==3);
                    sig_ep(2,j) = amp_c(fr==6);
                    noise_ep(1,j)=  (amp_c(find(fr==3)+1)+amp_c(find(fr==3)-1))/2;
                    noise_ep(2,j)=  (amp_c(find(fr==6)+1)+amp_c(find(fr==6)-1))/2;
                    
                end
                    ax_sub(i) = subplot(num_row,num_col,i);
                    plot(subj.stim_params.val{cond},sig_ep(1,:)./(noise_ep(1,:)),'LineWidth',2,'Marker','o','Color','k')
                    my_ylim1 = [my_ylim1; ax_sub(i).YLim];
                    hold on
                    plot(subj.stim_params.val{cond},sig_ep(2,:)./(noise_ep(2,:)),'LineWidth',2,'Marker','o','Color','r')
                    my_ylim2 = [my_ylim2; ax_sub(i).YLim];
...
%                     ylabel('SNR');
              ...
              linkaxes(ax_sub,'xy');
              ax_sub(j).YLim =  [min(min([my_ylim1; my_ylim2])) max(max([my_ylim1; my_ylim2]))];
              linkaxes(ax_sub,'xy');
              clear ax_sub;
              clear my_ylim;
              legend('3 HZ', '6 HZ');
        ...
        % plot SNR per trial avgeraging over all noise at adjacent
        % frequnecies over all epochs.
        figure;
              plt_titl = sprintf('%s, %s, %s\n, avg over all channels per trial, SNR, all noise \n ', subj.name,subj.stim_params.cond{cond},subj.date);
              suptitle(plt_titl);  
              num_epochs = size(tmp_ep,2);
              my_ylim1 = [];
              my_ylim2 = [];
         ...
         case 'chan'
                 figure;
                 plt_titl = sprintf('%s, %s, %s\n, avg over all trials SNR, all noise \n ', subj.name,subj.stim_params.cond{cond},subj.date);
                 suptitle(plt_titl);  
                 my_ylim1 = [];
                 my_ylim2 =[];
                 ...
                 for k =1:tot_chan
                     for e = 1: num_epoch
                        ...
                        plot(subj.stim_params.val{cond},sig_ep(1,:)./mean(noise_ep(1,:)),'LineWidth',2,'Marker','o','Color','k')
                        my_ylim1 = [my_ylim1; ax_sub(k).YLim];
                         hold on
                         plot(subj.stim_params.val{cond},sig_ep(2,:)./mean(noise_ep(2,:)),'LineWidth',2,'Marker','o','Color','r')
                         my_ylim2 = [my_ylim2; ax_sub(k).YLim];
                         set(gca, 'xlim', [min(subj.stim_params.val{cond})-0.5,max(subj.stim_params.val{cond})+0.5]);
                         set(gca, 'xscale', 'log');  
                         xlab = sprintf('%s',subj.stim_params.var{cond});
                         xlabel(xlab);
